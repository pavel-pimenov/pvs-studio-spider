...
- Ведите журнал работ в `SESSION.md` (в корне репозитория): после каждой сессии
  добавляйте запись с датой, тем, что сделано, результатами и следующими шагами.
  Файл коммитится в git и служит источником контекста для следующих сессий —
  перед началом работы читайте его, чтобы понять, что уже сделано.
